...
- `gctrial`: If `true`, run GC before the trial when enough memory has been allocated since the last collection. Defaults to `BenchmarkTools.DEFAULT_PARAMETERS.gctrial = true`.
- `gcsample`: If `true`, run GC between samples when enough memory has been allocated since the last collection. Defaults to `BenchmarkTools.DEFAULT_PARAMETERS.gcsample = false`.
